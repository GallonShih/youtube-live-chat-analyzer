...
# Stopwords 檔案路徑（Docker 容器內的路徑）
STOPWORDS_PATH = Path('/opt/airflow/text_analysis/cn_stopwords.txt')
...
def load_stopwords() -> Set[str]:
    ...
    if STOPWORDS_PATH.exists():
        ...
        print(f"Loaded {len(_stopwords_cache)} stopwords")
    else:
...
def tokenize_text(text: str, special_words: List[str], stopwords: Optional[Set[str]] = None) -> List[str]:
    """
    使用 jieba 進行斷詞
